...
export default {
  ...
  async [GET_ONE_OFFER_AUCTION_ASYNC]({ commit }, payload) {
    try {
      commit("formLoading/TOGGLE_FORM_LOADING");
      ...
      commit(GET_ONE_OFFER_AUCTION_SUCCESS, data);
    } catch (error) {
      console.log(error);
      commit(GET_ONE_OFFER_AUCTION_FAILURE, error);
    } finally {
      commit("formLoading/TOGGLE_FORM_LOADING");
    }
  },
