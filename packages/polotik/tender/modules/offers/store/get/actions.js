import {
  ...
  // get one methods
  GET_ONE_OFFER_TENDER_ASYNC,
  GET_ONE_OFFER_TENDER_SUCCESS,
  GET_ONE_OFFER_TENDER_FAILURE,
} from "./types";
...
export default {
  ...
  async [GET_ONE_OFFER_TENDER_ASYNC]({ commit }, payload) {
    try {
      commit("formLoading/TOGGLE_FORM_LOADING");
      ...
      commit(GET_ONE_OFFER_TENDER_SUCCESS, data);
    } catch (error) {
      console.log(error);
      commit(GET_ONE_OFFER_TENDER_FAILURE, error);
    } finally {
      commit("formLoading/TOGGLE_FORM_LOADING");
    }
  },
