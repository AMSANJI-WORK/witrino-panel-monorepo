          ...
          >
            <v-chip
              ...
              <span class="grey--text">
                &nbsp; تا {{ remainingDays }} روز دیگر</span
              >
            </v-chip>
            <v-chip
              :ripple="false"
            ...
            <div v-if="isCurrentUser" class="d-flex flex-row flex-wrap mt-2">
              ...
              <v-btn
                ...
                text
                :ripple="false"
                @click="deleteSaleAsync(dataSource.id)"
              >
                پایان حراجی
...
import Types from "@polotik/modules/Sale/Store/modules/root/types";
...
export default {
  ...
  methods: {
    ...mapActions({
      deleteSaleAsync: Types.DELETE_SALE_ASYNC,
    }),
    modify(itemId, path) {
      return this.$router.push({ path: `/sale/${itemId}/${path}` });
    },
  },
};
</script>
